...
class Attention(nn.Module):
    def __init__(self, embed_dim, num_heads, down_ratio=8, qkv_bias=False, 
        ...
        self.down_ratio = down_ratio
        if down_ratio > 1:
            self.sr = nn.Conv2d(embed_dim, embed_dim, kernel_size=down_ratio, stride=down_ratio)
            self.norm = nn.LayerNorm(embed_dim)
...
@register_model
def pvtformer_t(pretrained=False, **kwargs):
    model = PVTFormer(
        embed_dims=[64, 128, 320, 512], num_heads=[2, 4, 8, 16], mlp_ratios=[8, 8, 4, 4], qkv_bias=True, 
        norm_layer=partial(nn.BatchNorm2d, eps=1e-05), depths=[2, 2, 4, 2], down_ratios=[8, 4, 2, 1], **kwargs)
